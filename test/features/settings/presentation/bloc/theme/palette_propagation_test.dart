...
class _FakeThemeRepository implements ThemeRepository {
  ...
  @override
  Future<void> savePalette(ColorPalette palette) async => storedPalette = palette;
}
...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      final defaultPrimary =
          getPaletteColors(ColorPalette.defaultPalette).lightScheme.primary.toARGB32();
      expect(
      ...
      final oceanPrimary =
          getPaletteColors(ColorPalette.ocean).lightScheme.primary.toARGB32();
      ...
      final forestPrimary =
          getPaletteColors(ColorPalette.forest).lightScheme.primary.toARGB32();
      final texts2 = tester
          .widgetList<Text>(find.byType(Text))
          .map((t) => t.data ?? '')
          .toList();
  ...
  testWidgets(
    'Dark mode + palette switch uses correct dark scheme',
    (tester) async {
      final repo = _FakeThemeRepository()
      ...
      final defaultDarkPrimary = getPaletteColors(ColorPalette.defaultPalette)
      ...
      final oceanDarkPrimary =
          getPaletteColors(ColorPalette.ocean).darkScheme.primary.toARGB32();
      final texts = tester
          .widgetList<Text>(find.byType(Text))
          .map((t) => t.data ?? '')
          .toList();
  ...
  test('All 6 palettes produce distinct primary colors', () {
    ...
    for (final palette in ColorPalette.values) {
      results[palette] =
          AppTheme.buildLightTheme(palette).colorScheme.primary.toARGB32();
    }
    expect(results.values.toSet().length, ColorPalette.values.length,
